...
// ============================================================
// replace the prefix when adding to new plugin "yydev_top_btn_"
// ============================================================

$yydev_top_btn_notice_info_array = array(
	'plugin_name' => "Back To The Top Button", // the name of the plugin
	'developer_website' => "https://www.yydevelopment.com", // link to the developer website page
	'plugin_review_page' => "https://wordpress.org/plugins/back-to-the-top-button/#reviews", // link to the plugin support page 
	'plugin_support_link' => "https://wordpress.org/support/plugin/back-to-the-top-button/", // link to the plugin review page
	'plugin_donate_link' => "https://www.yydevelopment.com/coffee-break/?plugin=back-to-the-top-button", // link to the plugin donate page
	...
	'save_database_time_stamp_name' => "yydev_top_btn_timestamp",	// database input name to save data 
...
$plugin_db_timestamp = get_option($yydev_top_btn_notice_info_array['save_database_time_stamp_name']);
...
if( empty($plugin_db_timestamp) ) {
	...
	update_option($yydev_top_btn_notice_info_array['save_database_time_stamp_name'], $yydev_top_btn_notice_info_array['send_mail_in_days']);
	$plugin_db_timestamp = get_option($yydev_top_btn_notice_info_array['save_database_time_stamp_name']);
...
function yydev_top_btn_stop_notice_forever() {

	global $yydev_top_btn_notice_info_array;
	update_option($yydev_top_btn_notice_info_array['save_database_time_stamp_name'], 'stop');
	die(); // we have to end ajax functions with die();
	
} // function yydev_top_btn_stop_notice_forever() {

add_action( 'wp_ajax_yydev_top_btn_stop_notice_forever', 'yydev_top_btn_stop_notice_forever' );


// when the visitor ask to get the message in the future
function yydev_top_btn_stop_notice_for_now() {
...
add_action( 'wp_ajax_yydev_top_btn_stop_notice_for_now', 'yydev_top_btn_stop_notice_for_now' );
...
function yydev_top_btn_admin_notice($notice_info_array) {

	global $yydev_top_btn_notice_info_array;

	$plugin_name = $yydev_top_btn_notice_info_array['plugin_name'];
	$developer_website = $yydev_top_btn_notice_info_array['developer_website'];
	$plugin_review_page = $yydev_top_btn_notice_info_array['plugin_review_page'];
	$plugin_support_link = $yydev_top_btn_notice_info_array['plugin_support_link'];
	$plugin_donate_link = $yydev_top_btn_notice_info_array['plugin_donate_link'];
	$company_plugins_page = $yydev_top_btn_notice_info_array['company_plugins_page'];
	$icon_image_path = $yydev_top_btn_notice_info_array['icon_image_path'];
	$save_database_time_stamp_name = $yydev_top_btn_notice_info_array['save_database_time_stamp_name'];
...
<script>
	...
	jQuery(document).ready(function($){
		...
		$(document).on('click', '.yydev_top_btn_notice_style .yy-plugin-dismiss-forever', function() {
			$(this).parent().parent().animate({opacity: "0"}, 200, function() {

				$(this).css("display", "none");

				// use the function and update value using ajax
				var data = {'action': 'yydev_top_btn_stop_notice_forever'}; // var data = {
				jQuery.post(ajaxurl, data,  function(response) {});

			}); // $( relatedPostBox ).animate({opacity: "0"}, 1000, function() {
			return false;
		}); // $(document).on('click', '.yydev_top_btn_notice_style notice-dismiss', function() {

		// if the user click on ask me later
		$(document).on('click', '.yydev_top_btn_notice_style .yy-plugin-dismiss-for-now', function() {
			$(this).parent().parent().animate({opacity: "0"}, 200, function() {

				$(this).css("display", "none");

				// use the function and update value using ajax
				var data = {'action': 'yydev_top_btn_stop_notice_for_now'}; // var data = {
				jQuery.post(ajaxurl, data,  function(response) {});
		...
		// if the user click on the right close button or ask to stop showing message
		$(document).on('click', '.yydev_top_btn_notice_style .yy-review-plugin', function() {
			$(this).parent().parent().animate({opacity: "0"}, 200, function() {
...
<style>
	...
	.yydev_top_btn_notice_style .yy-bottom-plugin-name {
		color: #fff;
		padding: 3px 10px;
		position: absolute;
		bottom: -20px;
	...
	.yydev_top_btn_notice_style .notice-buttons {
		margin: 10px 0px 10px 0px;
	}

	.yydev_top_btn_notice_style .notice-buttons a {
		max-width: 100%;
		white-space: normal;
		margin: 4px 0px 4px 2px;
	}

	.yydev_top_btn_notice_style .yy-notice-dismiss {
		position: absolute;
		top: 0;
		left: auto;
		right: 1px;
	...
	.yydev_top_btn_notice_style .yy-notice-dismiss::before {
		background: 0 0;
		color: #72777c;
		content: "\f153";
		display: block;
	...
	.yydev_top_btn_notice_style .yy-plugin-icon {
		max-width: 85px;
		float: left;
	...
	<div class="yydev_top_btn_notice_style notice notice-info">
	...
	</div><!--yydev_top_btn_notice_style-->

<?php

} // function yydev_top_btn_admin_notice() {
...
if( !empty($plugin_db_timestamp) && ($plugin_db_timestamp != "stop") && ($plugin_db_timestamp < strtotime("now")) && ($yydev_stop_plugins_notice != 1) ) {
	...
	add_action( 'admin_notices', 'yydev_top_btn_admin_notice' );
